...
interface PrintContextType {
    processPrintJob: (job: any) => Promise<void>;
...
export const PrintProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Charger les paramètres de la boutique une seule fois au montage du Provider
    ...
    return (
        <PrintContext.Provider value={{ processPrintJob, checkPendingJobs, fetchSettings, loadingSettings, settings }}>
            {children}
        </PrintContext.Provider>
    );
};

// Le custom hook "usePrint" utilise maintenant le contexte partagé
export const usePrint = () => {
    const context = useContext(PrintContext);
    if (!context) {
        throw new Error("usePrint doit être utilisé à l'intérieur d'un PrintProvider");
    }
    return context;
};