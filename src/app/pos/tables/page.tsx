...
export default function TablesPage() {
    ...
    // 1. Hook de récupération
    const {data: zones = [], isLoading} = useQuery<Floor[]>({
        queryKey: ['floors'],
        queryFn: async () => {
    ...
    // On synchronise l'état local avec le résultat de la requête
    useEffect(() => {
    ...
    const filteredTables = activeZone?.tables ?? [];

    if (isLoading) return <div
        className="flex items-center justify-center h-screen font-black uppercase italic">Chargement du plan...</div>;
        ...
        <div className="flex flex-col h-full p-1 space-y-6">
            ...
            <div className="p-4 bg-card border-b flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <div
                            className="flex items-center gap-3 text-stone-500 group-hover:text-red-600 transition-colors">
                            <div className="flex flex-col items-end leading-none">
                                <span
                                    className="text-[9px] font-black uppercase tracking-widest opacity-50">Session</span>
                ...
                <span className={cn(
                  ...
                  {table.name}
                    ...
                    {table.seats} Pers.
                            ...
                            <div className={cn(
                                ...
                                <div className="flex flex-col">
                                    <span
                                        className="text-[8px] font-black uppercase text-slate-400 tracking-widest">Conso.</span>
                                    <span className={cn(
                                        "text-xs font-black",
                    ...
                    {table.total ? `${new Intl.NumberFormat().format(table.total)} FCFA` : "---"}
                            ...
                            <div
                                className="absolute inset-0 rounded-[2.5rem] bg-gradient-to-tr from-white/0 via-white/0 to-white/20 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"/>
                        </motion.div>
                    ))}
                </div>
            </div>
